...
class BitcoinTestFramework(metaclass=BitcoinTestMetaClass):
    ...
    def fill_node_addrman(self, *, node_index, address_types_to_add):
        ...
        for addr_type in address_types_to_add:
            for addr in ADDRESSES[addr_type]:
                res = self.nodes[node_index].addpeeraddress(address=addr, port=0 if addr.endswith(".i2p") else 26333, tried=False)
                if not res["success"]:
    ...
    def skip_if_no_bitcoin_bench(self):
        """Skip the running test if bench_bitweb has not been compiled."""
        if not self.is_bench_compiled():
            raise SkipTest("bench_bitweb has not been compiled")
    ...
    def is_bench_compiled(self):
        """Checks whether bench_bitweb was compiled."""
