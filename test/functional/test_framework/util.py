...
class Binaries:
    ...
    def __init__(self, paths, bin_dir, *, use_valgrind=False):
        ...
        self.valgrind_cmd = [
            ...
            "--trace-children=yes",  # Needed for 'bitweb' wrapper
    ...
    def bench_argv(self):
        "Return argv array that should be used to invoke bench_bitweb"
    ...
    def _argv(self, command, bin_path, need_ipc=False):
        ...
        It either uses the bitweb wrapper executable (if BITCOIN_CMD is set or
        ...
        (previous releases). Also, valgrind will only apply to the bitweb
        wrapper executable directly, not to the commands that `bitweb` calls.
        """
...
def initialize_datadir(dirname, n, chain, disable_autoconnect=True):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    write_config(os.path.join(datadir, "bitweb.conf"), n=n, chain=chain, disable_autoconnect=disable_autoconnect)
...
def get_temp_default_datadir(temp_dir: pathlib.Path) -> tuple[dict, pathlib.Path]:
    ...
    if platform.system() == "Windows":
        ...
        datadir = temp_dir / "Bitweb"
    else:
        env = dict(HOME=str(temp_dir))
        if platform.system() == "Darwin":
            datadir = temp_dir / "Library/Application Support/Bitweb"
        else:
            datadir = temp_dir / ".bitweb"
    return env, datadir
...
def append_config(datadir, options):
    with open(os.path.join(datadir, "bitweb.conf"), 'a') as f:
...
def get_auth_cookie(datadir, chain):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "bitweb.conf")):
        with open(os.path.join(datadir, "bitweb.conf"), 'r') as f:
            for line in f:
