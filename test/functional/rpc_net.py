...
class NetTest(BitcoinTestFramework):
    ...
    def test_addpeeraddress(self):
        ...
        assert_raises_rpc_error(-30, "Invalid IP address", node.addpeeraddress, address="", port=26333)
        assert_equal(node.getnodeaddresses(count=0), [])

        self.log.debug("Test that adding a non-IP/hostname fails (no DNS lookup allowed)")
        assert_raises_rpc_error(-30, "Invalid IP address", node.addpeeraddress, address="not_an_ip", port=26333)
