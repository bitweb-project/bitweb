...
class RawTransactionsTest(BitcoinTestFramework):
    ...
    def test_invalid_change_address(self):
        ...
        assert_raises_rpc_error(-5, "Change address must be a valid bitweb address", self.nodes[2].fundrawtransaction, rawtx, changeAddress='foobar')
    ...
    def test_option_feerate(self):
        self.log.info("Test fundrawtxn with explicit fee rates (fee_rate sat/vB and feeRate BTE/kvB)")
        node = self.nodes[3]
    ...
    def test_option_subtract_fee_from_outputs(self):
        ...
        # Test subtract fee from outputs with feeRate (BTE/kvB)
    ...
    def test_22670(self):
        ...
        # being accounted for. So the minimum relay fee is set to 0.1 BTE/kvB in this test.
    ...
    def test_feerate_rounding(self):
        ...
        # fail with insufficient funds rather than bitwebd asserting.
    ...
    def test_cannot_cover_fees(self):
        ...
        # Set up wallet with 2 utxos: 0.3 BTE and 0.15 BTE
        ...
        self.log.info("Attempt to send 0.45 BTE without SFFO")
        ...
        self.log.info("Send 0.45 BTE with SFFO")
        wallet.fundrawtransaction(rawtx, options={"subtractFeeFromOutputs":[0]})

        self.log.info("Attempt to send 0.45 BTE by restricting coin selection with minconf=6")
        ...
        self.log.info("Attempt to send 0.45 BTE preselecting 0.15 BTE utxo")
        rawtx = wallet.createrawtransaction(inputs=[{"txid": txid2, "vout": vout2}], outputs=[{default_wallet.getnewaddress(): 0.45}])
        assert_raises_rpc_error(-4, amount_with_fee_err_msg.format("0.00000042"), wallet.fundrawtransaction, rawtx, options={"fee_rate":1})

        self.log.info("Send 0.45 BTE preselecting 0.15 BTE utxo with SFFO")
        wallet.fundrawtransaction(hexstring=rawtx, options={"subtractFeeFromOutputs":[0]})

        self.log.info("Attempt to send 0.15 BTE using only the 0.15 BTE preselected utxo")
        ...
        self.log.info("Send 0.15 BTE using only the 0.15 BTE preselected utxo with SFFO")
