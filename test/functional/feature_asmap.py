...
Verify node behaviour and debug log when launching bitwebd in these cases:

1. `bitwebd` with no -asmap arg, using /16 prefix for IP bucketing

2. `bitwebd -asmap=<absolute path>`, using the unit test skeleton asmap

3. `bitwebd -asmap=<relative path>`, using the unit test skeleton asmap

4. `bitwebd -asmap/-asmap=` with no file specified, using the default asmap

5. `bitwebd -asmap` restart with an addrman containing new and tried entries

6. `bitwebd -asmap` with no file specified and a missing default asmap file

7. `bitwebd -asmap` with an empty (unparsable) default asmap file
...
class AsmapTest(BitcoinTestFramework):
    ...
    def test_default_asmap(self):
        ...
        for arg in ['-asmap', '-asmap=']:
            self.log.info(f'Test bitwebd {arg} (using default map file)')
            self.stop_node(0)
    ...
    def test_default_asmap_with_missing_file(self):
        self.log.info('Test bitwebd -asmap with missing default map file')
        self.stop_node(0)
