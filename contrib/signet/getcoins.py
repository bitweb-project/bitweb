...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing bitweb-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='bitweb-cli', help='bitweb-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Bitweb address to which the faucet should send')
...
parser.add_argument('bitweb_cli_args', nargs='*', help='Arguments to pass on to bitweb-cli (default: -signet)')

args = parser.parse_args()

if args.bitweb_cli_args == []:
    args.bitweb_cli_args = ['-signet']


def bitweb_cli(rpc_command_and_params):
    argv = [args.cmd] + args.bitweb_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = bitweb_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = bitweb_cli(['getnewaddress', 'faucet', 'bech32'])
