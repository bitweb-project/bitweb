...
bitwebcore.net and bitwebcore.net and compares them.
...
WORKINGDIR = "/tmp/bitweb_verify_binaries"
HASHFILE = "hashes.tmp"
...
def main(args):
    ...
    if not success:
        print("bitwebcore.net failed to provide signature file, "
              "but bitwebcore.net did?")
        print("wget output:")
        print(indent(output, '\t'))
    ...
    if not files_are_equal(sigfile1, sigfile2):
        print("bitwebcore.net and bitwebcore.net signature files were not equal?")
