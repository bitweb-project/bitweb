...
be used in Bitweb Core with the `-asmap` option.
...
to binary before being used with Bitweb Core.
...
the `bitweb-cli getnodeaddresses` command.
...
```
bitweb-cli getnodeaddresses 0 > addrs.json
```
