...
const std::vector<std::string> TEST_OPTIONS_DOC{
    ...
    // "bip94 (enforce BIP94 consensus rules)", /* Bitweb Params */
};
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\Bitweb
    //   new: C:\Users\Username\AppData\Local\Bitweb
    // macOS: ~/Library/Application Support/Bitweb
    // Unix-like: ~/.bitweb
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "Bitweb";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "Bitweb";
#else
    fs::path pathRet;
