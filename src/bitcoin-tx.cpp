...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        std::string strUsage = PACKAGE_NAME " bitweb-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.IsArgSet("-version")) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "Usage:  bitweb-tx [options] <hex-tx> [commands]  Update hex-encoded bitweb transaction\n"
                "or:     bitweb-tx [options] -create [commands]   Create hex-encoded bitweb transaction\n"
                "\n";
