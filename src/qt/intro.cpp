...
bool Intro::showIfNeeded(bool& did_show_intro, int64_t& prune_MiB)
{
     ...
     * (to be consistent with bitwebd behavior)
     */
    if(dataDir != GUIUtil::getDefaultDataDirectory()) {
...
void Intro::UpdatePruneLabels(bool prune_checked)
{
    ...
    static constexpr uint64_t nPowTargetSpacing = 5 * 60;  // from chainparams, which we don't have at this stage  /* Bitweb Params */
    ...
    ui->sizeWarningLabel->setText(
        tr("%1 will download and store a copy of the Bitweb block chain.").arg(CLIENT_NAME) + " " +
