# PSBT Howto for Bitweb Core

Since Bitcoin Core 0.17, an RPC interface exists for Partially Signed Bitweb
...
PSBT is an interchange format for Bitweb transactions that are not fully signed
...
Overall, the construction of a fully signed Bitweb transaction goes through the
following steps:
...
- An **Extractor** produces a valid Bitweb transaction (in network format)
...
#### Multisig with multiple Bitweb Core instances
...
Bitweb Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `bitweb-cli` is used.
