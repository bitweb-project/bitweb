# User-space, Statically Defined Tracing (USDT) for Bitweb Core

Bitweb Core includes statically defined tracepoints to allow for more
...
**Note**: Bitweb Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Bitweb Core
...
Multiple tools can list the available tracepoints in a `bitwebd` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Bitweb Core, use `info probes` in `gdb`:

```
$ gdb ./build/bin/bitwebd
…
(gdb) info probes
Type Provider   Name             Where              Semaphore Object
stap net        inbound_message  0x000000000014419e 0x0000000000d29bd2 /build/bin/bitwebd
stap net        outbound_message 0x0000000000107c05 0x0000000000d29bd0 /build/bin/bitwebd
stap validation block_connected  0x00000000002fb10c 0x0000000000d29bd8 /build/bin/bitwebd
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Bitweb Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./build/bin/bitwebd | grep NT_STAPSDT -A 4 -B 2
