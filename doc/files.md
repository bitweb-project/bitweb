...
The data directory is the default location where the Bitweb Core files are stored.
...
---------|--------------------
Linux    | `$HOME/.bitweb/`
macOS    | `$HOME/Library/Application Support/Bitweb/`
Windows  | `%LOCALAPPDATA%\Bitweb\` <sup>[\[1\]](#note1)</sup>

2. A custom data directory path can be specified with the `-datadir` option.

3. All content of the data directory, except for `bitweb.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`blocks/`          | `blkNNNNN.dat`<sup>[\[2\]](#note2)</sup> | Actual Bitweb blocks (dumped in network format, 128 MiB per file)
...
`./`               | `bitweb.conf`        | User-defined [configuration settings](bitweb-conf.md) for `bitwebd` or `bitweb-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
`./`               | `bitwebd.pid`        | Stores the process ID (PID) of `bitwebd` or `bitweb-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `bitwebd` or `bitweb-qt`; can be specified by `-debuglogfile` option
...
`./`               | `settings.json`       | Read-write settings set through GUI or RPC interfaces, augmenting manual settings from [bitweb.conf](bitweb-conf.md). File is created automatically if read-write settings storage is not disabled with `-nosettings` option. Path can be specified with `-settings` option
...
`bitweb-qt` uses [`QSettings`](https://doc.qt.io/qt-6/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-6/qsettings.html#locations-where-application-settings-are-stored).

## Legacy subdirectories and files

These subdirectories and files are no longer used by Bitweb Core:
...
`blktree/`     | Blockchain index; replaced by `blocks/index/` in [0.8.0](/doc/bitcoin-release-notes/release-notes-0.8.0.md#improvements) | [PR #2231](https://github.com/bitcoin/bitcoin/pull/2231), [`8fdc94cc`](https://github.com/bitcoin/bitcoin/commit/8fdc94cc8f0341e96b1edb3a5b56811c0b20bd15)
...
`addr.dat`     | Peer IP address BDB database; replaced by `peers.dat` in [0.7.0](/doc/bitcoin-release-notes/release-notes-0.7.0.md) | [PR #1198](https://github.com/bitcoin/bitcoin/pull/1198), [`928d3a01`](https://github.com/bitcoin/bitcoin/commit/928d3a011cc66c7f907c4d053f674ea77dc611cc)
`onion_private_key` | Cached Tor onion service private key for `-listenonion` option. Was used for Tor v2 services; replaced by `onion_v3_private_key` in [0.21.0](/doc/bitcoin-release-notes/release-notes-0.21.0.md) | [PR #19954](https://github.com/bitcoin/bitcoin/pull/19954)
...
This table describes the files installed by Bitweb Core across different platforms.
...
| bin/bitweb-tx                                             | Tool for creating and modifying transactions                                |
| bin/bitweb-util                                           | Miscellaneous utilities                                                     |
| bin/bitweb-wallet                                         | Bitweb wallet tool                                                         |
| bin/bitwebd                                               | Bitweb node and wallet daemon                                              |
...
| *libexec/bench_bitweb*                                    | Benchmarking tool for measuring node performance                            |
| *libexec/bitweb-chainstate*                               | Tool to validate and connect blocks                                         |
| libexec/bitweb-gui                                        | IPC-enabled alternative to `bitweb-qt`                                     |
| libexec/bitweb-node                                       | IPC-enabled alternative to `bitwebd`                                       |
| libexec/test_bitweb                                       | Unit test binary                                                            |
| *libexec/test_bitweb-qt*                                  | GUI-specific unit tests                                                     |
| share/man/man1/                                            | Man pages for command-line tools like `bitweb-cli`, `bitwebd`, and others |
...
- README and bitweb.conf files are included in binary releases but not installed in source builds.
- On Windows, binaries have a `.exe` suffix (e.g., `bitweb-cli.exe`).
...
There have been multiple reports of database corruption and data loss when using this filesystem with Bitcore Core, see [Issue #31454](https://github.com/bitcoin/bitcoin/issues/31454) for more details.
